...
async function processMessage(message: any): Promise<void> {
  ...
  try {
    ...
    messageLogger.info({ job_id, task_type, payload_key }, 'Processing coding task');
    ...
    // Run the coder agent with modern dark theme
    const result = await runCodingAgent(enrichedPayload);
    
    // Store the coder result in S3
    const resultKey = `${job_id}/coder-result.json`;
    ...
    messageLogger.info({ resultKey }, 'Coder result stored in S3');
    ...
    await coderQueue.deleteMessage(message.ReceiptHandle);
    messageLogger.info({ job_id }, 'Coding task completed successfully');

  } catch (error) {
    messageLogger.error({ error }, 'Failed to process coding task');
...
async function pollForMessages(): Promise<void> {
  if (!CODER_QUEUE_URL) {
    logger.error('CODER_QUEUE_URL environment variable is not set');
    process.exit(1);
  }

  logger.info({ queueUrl: CODER_QUEUE_URL }, 'Starting coder agent worker');

  while (true) {
    try {
      // Poll for messages
      const messages = await coderQueue.receiveMessages(1, 20);
