...
async function processMessage(message: any): Promise<void> {
  ...
  try {
    ...
    messageLogger.info({ job_id, task_type, payload_key }, 'Processing design task');
    ...
    const result = await runDesignerAgent(enrichedPayload);
    ...
    await designerQueue.deleteMessage(message.ReceiptHandle);
    messageLogger.info({ job_id }, 'Design task completed successfully');

  } catch (error) {
    messageLogger.error({ error }, 'Failed to process design task');
...
async function pollForMessages(): Promise<void> {
  if (!DESIGNER_QUEUE_URL) {
    logger.error('DESIGNER_QUEUE_URL environment variable is not set');
    process.exit(1);
  }

  logger.info({ queueUrl: DESIGNER_QUEUE_URL }, 'Starting designer agent worker');

  while (true) {
    try {
      // Poll for messages
      const messages = await designerQueue.receiveMessages(1, 20);
